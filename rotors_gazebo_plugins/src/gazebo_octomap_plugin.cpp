...
void OctomapFromGazeboWorld::FloodFill(
  ...
  std::stack<ignition::math::Vector3d > to_check;
  to_check.push(ignition::math::Vector3d (seed_point.X(), seed_point.Y(), seed_point.Z()));

  while (to_check.size() > 0) {
    //octoignition::math::Vector3d p = to_check.top();
    ignition::math::Vector3d p = to_check.top();
    ...
    if ((p.X() > bounding_box_origin.X() - bounding_box_lengths.X() / 2) &&
        ...
        (p.Z() > bounding_box_origin.Z() - bounding_box_lengths.Z() / 2) &&
        (p.Z() < bounding_box_origin.Z() + bounding_box_lengths.Z() / 2) &&
        (!octomap_->search(p.X(), p.Y(), p.Z()))) {
      octomap_->setNodeValue(p.X(), p.Y(), p.Z(), 0);
      to_check.pop();
      ...
      //to_check.push(octoignition::math::Vector3d (p.x(), p.y(), p.z() - leaf_size));
      to_check.push(ignition::math::Vector3d (p.X() + leaf_size, p.Y(), p.Z()));
      to_check.push(ignition::math::Vector3d (p.X() - leaf_size, p.Y(), p.Z()));
      to_check.push(ignition::math::Vector3d (p.X(), p.Y() + leaf_size, p.Z()));
      to_check.push(ignition::math::Vector3d (p.X(), p.Y() - leaf_size, p.Z()));
      to_check.push(ignition::math::Vector3d (p.X(), p.Y(), p.Z() + leaf_size));
      to_check.push(ignition::math::Vector3d (p.X(), p.Y(), p.Z() - leaf_size));
